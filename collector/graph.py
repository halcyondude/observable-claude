...
DDL_STATEMENTS = [
    ...
    "CREATE NODE TABLE IF NOT EXISTS Message (message_id STRING, agent_id STRING, session_id STRING, role STRING, sequence INT64, timestamp STRING, content_preview STRING, content_bytes INT64, synthetic BOOLEAN, PRIMARY KEY (message_id))",
...
def materialize_message(
    ...
    *,
    ...
    """Create a Message node, HAS_MESSAGE edge, and NEXT chain link in the graph."""
    ts = _now_iso()
    ...
    # NEXT chain: link from the previous message for this agent (if any)
    if sequence > 0:
        try:
            result = conn.execute(
                "MATCH (a:Agent {agent_id: $aid})-[:HAS_MESSAGE {sequence: $prev_seq}]->(prev:Message) "
                "RETURN prev.message_id",
                parameters={"aid": agent_id, "prev_seq": sequence - 1},
            )
            prev_rows = result.get_all()
            if prev_rows:
                prev_mid = prev_rows[0][0]
                conn.execute(
            ...
            logger.debug("Failed to create NEXT link for message %s", message_id)
...
def _handle_subagent_start(conn: Connection, event: dict) -> None:
    ...
    if parent_agent_id:
        ...
        conn.execute(
            ...
            parameters={
                "pid": parent_agent_id, "aid": agent_id,
                "prompt": prompt, "depth": depth, "ts": ts,
            },
        )
    elif session_id:
        conn.execute(
        ...
        conn.execute(
            ...
            parameters={
                ...
                "prompt": prompt, "depth": int(depth), "ts": ts,
            },
        )
...
def _handle_pre_tool_use(conn: Connection, event: dict) -> None:
    ...
    conn.execute(
        ...
        parameters={
            ...
            "tuid": tool_use_id, "input": tool_input, "ts": ts,
        },
    )
...
def _handle_post_tool_use(conn: Connection, event: dict, failed: bool = False) -> None:
    ...
    conn.execute(
        ...
        parameters={
            "tuid": tool_use_id, "ts": ts,
            "dur": duration_ms, "status": status, "resp": tool_response,
        },
    )
...
def get_session_graph(conn: Connection, session_id: str) -> dict:
    ...
    # Session node
    result = conn.execute(
        "MATCH (s:Session {session_id: $sid}) RETURN s.session_id, s.cwd, s.start_ts, s.end_ts, s.branch",
        parameters={"sid": session_id},
    )
    for row in result.get_all():
        nodes.append({
            "data": {
                "id": row[0], "label": "Session", "type": "Session",
                "session_id": row[0], "cwd": row[1],
                "start_ts": row[2], "end_ts": row[3], "branch": row[4],
            }
        })
    ...
    for row in result.get_all():
        nodes.append({
            "data": {
                "id": row[0], "label": row[1] or "Agent", "type": "Agent",
                "agent_id": row[0], "agent_type": row[1],
                "start_ts": row[2], "end_ts": row[3], "status": row[4],
            }
        })

    # Tool nodes
    result = conn.execute(
        "MATCH (a:Agent {session_id: $sid})-[r:INVOKED]->(t:Tool) RETURN DISTINCT t.name",
        parameters={"sid": session_id},
    )
    for row in result.get_all():
        nodes.append({
            "data": {"id": f"tool:{row[0]}", "label": row[0], "type": "Tool", "name": row[0]}
        })
    ...
    for row in result.get_all():
        edges.append({
            "data": {
                "source": row[0], "target": row[1], "label": "SPAWNED",
                "prompt": row[2], "depth": row[3], "spawned_at": row[4],
            }
        })

    # SPAWNED edges: Agent -> Agent
    result = conn.execute(
    ...
    for row in result.get_all():
        edges.append({
            "data": {
                "source": row[0], "target": f"tool:{row[1]}", "label": "INVOKED",
                "tool_use_id": row[2], "start_ts": row[3], "end_ts": row[4],
    ...
    for row in result.get_all():
        ...
        if agent_id in agents:
            agents[agent_id]["tool_events"].append({
                "tool_use_id": row[1], "tool_name": row[2],
                "start_ts": row[3], "end_ts": row[4], "status": row[5],
            })
